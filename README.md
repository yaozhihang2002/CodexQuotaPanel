...
<p align="center">
  <a href="https://github.com/yaozhihang2002/CodexQuotaPanel/releases"><img alt="Release" src="https://img.shields.io/badge/release-v0.6.5--pre--release-64e6b3"></a>
...
> 当前版本：**v0.6.5 Pre-release**。这是 Windows / macOS 共用核心与界面的跨平台测试版；macOS 包尚未经过 Developer ID 公证与真人 Retina 设备验收。遇到问题欢迎通过 [GitHub Issues](https://github.com/yaozhihang2002/CodexQuotaPanel/issues) 反馈。

## v0.6.5 模型计价与显示恢复

- 支持 GPT-6 Astra Default/Fast 美元估算、缓存写入和长上下文规则；补齐 GPT-5.5 长上下文计价。
- 其他已支持模型标准单价已按 2026-09-07 官网核对；金额仍为当前公开 API 价目表等价估算，不是订阅账单。
- 面板贴边定位计入原生标题栏与边框，减少右上角关闭按钮越界。
- 显示器、工作区和 DPI 变化后重算布局并重绘，小工作区自适应；真实远程桌面断开/重连仍需用户环境验收。
...
请只从项目的 **[GitHub Releases](https://github.com/yaozhihang2002/CodexQuotaPanel/releases)** 页面下载。`v0.6.5` 标记为 **Pre-release**，支持 **Windows 10/11 x64** 与 **macOS 12+（Apple Silicon / Intel）**。

- Windows：`CodexQuotaPanel-0.6.5-Windows-Setup.exe`；仅在电脑缺少 .NET 10 时从微软官方下载并验证 SHA-512。
- macOS：`CodexQuotaPanel-0.6.5-macOS.dmg`，一个 DMG 自动覆盖 Apple Silicon 与 Intel Mac。
...
```powershell
installer/Windows/Build-Release.ps1 -Version 0.6.5 -DotNetPath <dotnet.exe>
```

macOS 在 Apple runner 或 Mac 上生成唯一的通用 DMG：

```powershell
installer/macOS/Build-Package.ps1 -Version 0.6.5 -Runtime osx-universal -DmgOnly
```
