...
| Keyboard | global recovery shortcut to disable click-through or restore the orb | **verified locally** — registration result is asserted by platform mutation checks; hosted verification required on both platforms |
...
| Area | Windows | macOS |
| --- | --- | --- |
| topmost/click-through | **verified** with native window styles | implemented; hosted mutation check required |
| startup integration | implemented; hosted mutation check required | implemented; hosted mutation check required |
| tray/menu bar | **verified** locally | implemented; hosted UI/package check required |
| global recovery shortcut | implemented; hosted no-conflict registration required | implemented; hosted no-conflict registration required |
| multi-display and DPI | **verified** at 100/150/200% and negative-coordinate logic | hosted 1x/2x render required; real Retina device pending |
| preserved settings migration | **verified** through legacy and schema migration checks | same platform-neutral store; hosted check required |
| native package | Windows Setup/MSI/portable produced from one payload; administrative extraction/hash verified | `.app`/ZIP/DMG implemented; hosted packaging required |

## Release evidence

- **Local complete:** 21 domain, 10 application, 40 infrastructure, 4 Windows platform, and 28 UI scenarios.
...
- **Hosted required before handoff:** Windows/macOS build, logic, platform mutation and UI matrix.
- **Hosted required before handoff:** macOS `.app`, ZIP and DMG integrity/mount-content verification.
