...
public sealed class DashboardWindow : Window
{
    ...
    public void ApplyPresentation(QuotaPresentation presentation)
    {
        ...
        if (windows.Count > 0)
        {
            ...
            _summaryOrb.PrimaryLabel = ShortLabel(outerWindow.WindowMinutes);
            _summaryOrb.SecondaryRemainingPercent = innerWindow?.ClampedRemainingPercent ?? double.NaN;
            _summaryOrb.SecondaryLabel = innerWindow is null ? string.Empty : ShortLabel(innerWindow.WindowMinutes);
    ...
    private Control BuildContent()
    {
        ...
        var header = new Grid { ColumnDefinitions = new ColumnDefinitions("*,Auto"), Margin = new Thickness(20, 15, 16, 12) };
