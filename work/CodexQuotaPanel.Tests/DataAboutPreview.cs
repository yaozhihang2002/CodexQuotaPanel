...
internal static class DataAboutPreview
{
    internal static void Run(string outputPath)
    {
        ...
        using var settings = new SettingsForm(preferences, startupEnabled: false, snapshot: null,
            diagnostics: "CodexQuotaPanel v0.5.1 Pre-release");
        settings.Show();
        ...
        settings.Hide();
        Console.WriteLine($"PASS data & about preview | maximum typography + v0.5.1 Pre-release | {Path.GetFullPath(outputPath)}");
    }
